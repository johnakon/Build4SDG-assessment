...
const commonFunction = (currentlyInfected, data) => {
  const { periodType, timeToElapse, totalHospitalBeds, region } = data;
  ...
  const hospitalBedsByRequestedTime = Math.trunc(
    hospitalBedAvailable - severeCasesByRequestedTime
  );
  ...
  const avgDollarXIncomePopXtime =
    avgDailyIncomePopulation * avgDailyIncomeInUSD;
  const dollarOut =
    (infectionsByRequestedTime * avgDollarXIncomePopXtime) / timeInDays;
